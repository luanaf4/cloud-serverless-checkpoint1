...
const {createAIProvider, mockEnrich} = require('./ai');
...
async function handler(event) {
  ...
  try { const result = await processOrderEvent(event, logger, createAIProvider(), createPublisherFromEnv()); logger.info('order_processing_finished', {metricName: 'orders_succeeded', orderId: result.orderId, durationMs: Date.now() - startedAt}); return result; }
