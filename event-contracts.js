...
function createEvent(eventType, data, {eventId = `evt-${Date.now()}`, correlationId, idempotencyKey} = {}) {
  return {
    ...
    source: 'orders.gcp',
    correlationId: correlationId || idempotencyKey || null,
...
function validateEvent(event) {
  ...
  for (const field of ['eventId', 'eventType', 'schemaVersion', 'source', 'data']) {
    if (!event[field]) throw new Error(`Evento sem ${field}.`);
  }
  return event;
}
